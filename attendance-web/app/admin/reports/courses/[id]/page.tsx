...
export default function AdminSingleCourseReportPage() {
  ...
  const [editingWeekRemark, setEditingWeekRemark] = useState<{
    weekNumber: number;
    date: string;
    note: string;
  } | null>(null);
  ...
  const formatTimeString = (timeValue: any) => {
    if (!timeValue) return '';
  ...
  const fetchCourseDetailsAndHistory = useCallback(async () => {
    ...
    try {
      ...
      if (courseJson.success && courseJson.data?.course) {
        setCourseInfo(courseJson.data.course);
        setTotalStudentsCount(courseJson.data.course.students?.length || 0);
      } else {
        const fallbackRes = await fetch(`/api/courses/${courseId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const fallbackJson = await fallbackRes.json();
        if (fallbackJson.success && fallbackJson.data) {
          setCourseInfo(fallbackJson.data);
          setTotalStudentsCount(fallbackJson.data.students?.length || 0);
        }
      }
      ...
      if (resHistory && resHistory.ok) {
        ...
        // เรียงลำดับตามวันที่เรียนจริง (date) และช่วงเวลา (timeSlot) เสมือนหน้าอาจารย์
  ...
  const handleSaveWeekNote = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setWeeksSummaryData(prev => prev.map((w, idx) => {
        ...
        localStorage.setItem(`course_${courseId}_week_${editingWeekRemark.weekNumber}_note`, editingWeekRemark.note);
        setWeeksSummaryData(prev => prev.map((w, idx) => {
          if (idx + 1 === editingWeekRemark.weekNumber) {
            return { ...w, note: editingWeekRemark.note };
          }
  ...
  const getTimeSlotLabel = (slot: string) => {
    if (!slot) return '';
    if (slot.includes('09:') || slot.includes('08:') || slot.includes('10:') || slot.includes('11:') || slot.includes('12:')) {
      return `คาบเช้า (${slot} น.)`;
    }
    if (slot.includes('13:') || slot.includes('14:') || slot.includes('15:') || slot.includes('16:')) {
  ...
  // ประมวลผลข้อมูลสำหรับพิมพ์แบบฟอร์ม (จัดกลุ่ม 15 สัปดาห์ตรงกับฝั่งอาจารย์ 100%)
  const printStudentsData = useMemo(() => {
    ...
    // เรียงประวัติทั้งหมดตามวันที่เรียนจริง (date) และช่วงเวลา (timeSlot)
    ...
    return baseStudents.map((student: any) => {
      ...
      standardWeeks.forEach((weekSession: any, wIdx: number) => {
        ...
        // รวบรวม record ของนักศึกษาคนนี้จากทุกรอบในสัปดาห์นี้
        for (const s of weekSession.allSessions) {
          const records = s.records || s.attendances || [];
        ...
        if (studentRecordsInWeek.length > 0) {
          // หากมีสถานะที่ระบุเป็น ลา / มาสาย / มาเรียน ชัดเจน (เช่น จากการแก้ไขของอาจารย์)
      ...
      // คิดเปอร์เซ็นต์ตามเกณฑ์: สาย 2 = ขาด 1, ลา 2 = ขาด 1
      const divisor = standardWeeks.length || 15;
      ...
      const percent = divisor > 0 ? Math.round((actualAttended / divisor) * 100) : 0;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0f7f4] font-sans text-slate-800">
      ...
      <div className="print:hidden flex flex-col flex-1">
        ...
        {/* Modal Popup: แก้ไขหมายเหตุ / บันทึกการสอนชดเชยของสัปดาห์ */}
        {editingWeekRemark && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl border border-slate-100 animate-in zoom-in-95 duration-200">
