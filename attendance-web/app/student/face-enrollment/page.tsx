...
async function cropAllFacesFromFile(file: File): Promise<File[]> {
  try {
    ...
    // 3. วนลูปครอบตัดใบหน้าทุกหน้าที่ตรวจพบบนภาพ
...
export default function FaceEnrollmentPage() {
  ...
  // State สำหรับแสดงผลการเก็บ Multi-Frame
  ...
  useEffect(() => {
    ...
    if (!savedUser) {
      showToast('error', 'จำเป็นต้องเข้าสู่ระบบ', 'กรุณาเข้าสู่ระบบก่อนทำการลงทะเบียนใบหน้า', () => {
    ...
    setStatus(`สวัสดี ${initialName} กรุณาเลือกวิธีลงทะเบียนใบหน้าด้านล่าง`);
    ...
    const fetchLatestProfile = async () => {
      try {
        ...
        if (resJson.success && resJson.data) {
          ...
          setStatus(`สวัสดี ${freshName} กรุณาเลือกวิธีลงทะเบียนใบหน้าด้านล่าง`);
        }
      } catch (err) {
  ...
  const handleFinalSave = async (vectorsToSave: any[]) => {
    ...
    try {
      ...
      // กรณีอัปโหลดรูปภาพ (รองรับการตัดแบ่งหน้าทั้งหมดอัตโนมัติจากทุกรูปที่เลือก)
      if (files && files.length > 0) {
        ...
        if (totalValidFaces === 0) {
          throw new Error('ไม่พบใบหน้าที่ชัดเจนในรูปภาพที่เลือกเลย กรุณาตรวจสอบความสว่างและมุมมองของภาพ');
        }
      ...
      if (dbResult.success) {
        setStatus(`ลงทะเบียนใบหน้าสำเร็จเรียบร้อย (บันทึกสำเร็จ ${allFinalVectors.length} รูป)`);
        showToast('success', 'ลงทะเบียนใบหน้าเรียบร้อย', `บันทึกเวกเตอร์ใบหน้าสมบูรณ์ ${allFinalVectors.length} มุมมอง ระบบจะพาคุณไปยังหน้าหลัก`, () => {
          router.replace('/student/dashboard');
        });
      } else {
        throw new Error(dbResult.error || 'บันทึกข้อมูลไม่สำเร็จ');
      }
    } catch (err: any) {
      setStatus(`ข้อผิดพลาด: ${err.message}`);
      showToast('error', 'เกิดข้อผิดพลาด', err.message || 'ไม่สามารถบันทึกข้อมูลใบหน้าได้');
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200/80">
          ...
          <div className={`${regMode === 'upload' ? 'block' : 'hidden'} animate-in fade-in space-y-4`}>
            ...
            <div className="bg-emerald-50/60 border border-emerald-100 rounded-2xl p-5 md:p-6">
              ...
              <p className="text-xs md:text-sm text-emerald-900 mb-5 leading-relaxed font-bold">
                ...
                <div className="flex flex-wrap justify-center gap-2 mt-4">
                  {previews.map((src, i) => (
                    <div key={i} className="relative">
                      <img src={src} alt={`preview-${i}`} className="w-16 h-16 object-cover rounded-xl border border-slate-200 shadow-2xs" />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl border border-slate-100 animate-in zoom-in-95 duration-200 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              คุณต้องการบันทึกข้อมูลรูปภาพนี้สำหรับนักศึกษา <br />
